        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4'>
          {items
            ...
            .map((item) => (
              <div
                key={item.id}
                className='relative border-2 rounded-xl shadow-lg p-4'
              >
                ...
                <Button
                  size={"icon"}
                  onClick={() => removeTripFromCart(item.id)}
                  className='absolute top-2 right-2   bg-transparent rounded-xl text-red-500  hover:bg-white cursor-pointer'
                >
                  ❌
                </Button>
              </div>
